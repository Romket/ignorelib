...
namespace ignorelib
{
    ...
    std::vector<fs::path> IgnoreFile::ListIgnoredFull(const fs::path& dir) const
    {
        ...
        walk(dir,
             [&](const fs::path& path, const fs::file_type& type)
             {
    ...
    bool IgnoreFile::ignoredUtil(const fs::path& path, fs::file_type type,
                                 bool isFullMatch) const
    {
        ...
        for (const Pattern& pattern : _patterns)
        {
            ...
            {
                MatchesInfo info {
                    pathStr.substr(sepInfo.Separators[i],
    ...
    std::vector<size_t> IgnoreFile::findSeparators(std::string_view sv)
    {
        ...
        separators.reserve(!sv.empty() ? sv.size() - 1 : 0);
